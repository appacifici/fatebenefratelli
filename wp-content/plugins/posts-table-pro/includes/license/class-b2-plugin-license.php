...
if ( ! class_exists( 'Barn2_Plugin_License' ) ) {
	...
	class Barn2_Plugin_License {
						...
						switch ( $license_data->error ) {
							case 'expired' :
								$error_message	 = sprintf(
									__( 'Your license key expired on %s.', 'posts-table-pro' ), date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
								);
								break;
							case 'revoked' :
								$error_message	 = __( 'Your license key has been disabled.', 'posts-table-pro' );
								break;
							case 'missing' :
							case 'item_name_mismatch' :
								$error_message	 = __( 'Your license key is invalid.', 'posts-table-pro' );
								break;
							case 'invalid' :
							case 'site_inactive' :
								$error_message	 = __( 'Your license is not active for this URL.', 'posts-table-pro' );
								break;
							case 'no_activations_left':
								$error_message	 = __( 'Your license key has reached its activation limit.', 'posts-table-pro' );
								break;
							default :
								$error_message	 = __( 'There was an error activating your license, please try again.', 'posts-table-pro' );
								$result			 = 'error';
								break;
						}
					} // if license invalid
		...
		public function get_license_key_admin_message() {
			...
			$deactivate_button	 = sprintf( '<button type="submit" class="button" name="deactivate_key" value="%1$s" style="margin:3px 0 0 8px;">%2$s</button>', esc_attr( $this->license_key_option ), __( 'Deactivate', 'posts-table-pro' ) );

			if ( $this->is_valid() ) {
				$message = sprintf( '<span style="color:green;">✓ %s</span>', __( 'License key successfully activated.', 'posts-table-pro' ) ) . $deactivate_button;
			} elseif ( $this->is_deactivated() ) {
				$message = __( 'License key successfully deactivated. Click Save to reactivate.', 'posts-table-pro' );
			} elseif ( $this->failed_deactivation() ) {
				$message = sprintf( '<span style="color:red;">%s</span>', __( 'There was an error deactivating your license key, please try again.', 'posts-table-pro' ) ) . $deactivate_button;
			} elseif ( $this->is_invalid() ) {
				$message = $this->get_license_error();
				if ( ! $message ) {
					$message = __( 'There was an error activating your license, please try again.', 'posts-table-pro' );
				}
