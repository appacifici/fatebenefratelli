...
if (!function_exists('om_breadcrumbs')) {
	function om_breadcrumbs($caption = '', $before = '<div class="breadcrumbs">', $after = '</div>', $separator = ' / ')
	{
		...
		if (is_home()) {
			...
			if ($show_last)
				$out[] = om_get_archive_page_titolo();
			...
			if ($show_last)
				$out[] = ($post_type == "Attività Ospedaliere" and isset(get_option("fbf_settings")['fbf_nome_attivita_sanitaria']) and get_option("fbf_settings")['fbf_nome_attivita_sanitaria'] != "") ? get_option("fbf_settings")['fbf_nome_attivita_sanitaria'] : $post_type;
			om_breadcrumbs_add_parents($out, $post);
		} elseif (is_page()) {
			if ($show_last)
				$out[] = $post->post_title;
			...
			if ($post->post_type == 'attivita-ospedaliere') {
				if ($show_last)
					$out[] = $post->post_title;
				$out[] = '<a href="' . get_post_type_archive_link('attivita-ospedaliere') . '">' .
					((isset(get_option("fbf_settings")['fbf_nome_attivita_sanitaria']) and get_option("fbf_settings")['fbf_nome_attivita_sanitaria'] != "") ? get_option("fbf_settings")['fbf_nome_attivita_sanitaria'] : "Attività Ospedaliere") . '</a>';
				om_breadcrumbs_add_parents($out, $post);
