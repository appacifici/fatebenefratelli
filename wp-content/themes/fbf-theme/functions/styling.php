...
?>
		...
		************************/
		...
		$colore_sede = get_option('fbf_settings')['fbf_colore_sede'] ? get_option('fbf_settings')['fbf_colore_sede'] : "#00214C";
		...
		background-image: linear-gradient(45deg, #3A506B00 0%, <?php echo $colore_sede; ?> 20%);
		background-image: -ms-linear-gradient(left, #3A506B 0%, <?php echo $colore_sede; ?> 20%);
		}

		@media only screen and (max-width: 767px) {
		.headline-text {
		background: <?php echo $colore_sede; ?> !important;
		}
		}

		.immagine-primo-piano:before,
		.elenco-articoli-in-evidenza::before {
		background-color: <?php echo $colore_sede; ?> ;
		}
...
if (!function_exists('om_theme_styling')) {
	...
	{
		if (get_option(OM_THEME_PREFIX . 'use_inline_css') == 'true' || !file_exists(get_template_directory_uri() . om_custom_style_file_path())) {
