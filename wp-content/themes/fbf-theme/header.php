			...
			<header>
				...
				<div class="headline block-full<?php if ($icons_html) echo ' with-socials' ?>">
					...
					<div class="headline-text">
						<?php
						echo get_option(OM_THEME_PREFIX . 'intro_text');

						$options = get_option('fbf_settings');

						if (isset($options['fbf_portale_provincia']) and $options['fbf_portale_provincia'] != "") {
						?>
							<a href="<?= $options['fbf_portale_provincia'] ?>"><span>Portale della PROVINCIA ROMANA</span></a>
						<?php
						}
						if (isset($options['fbf_link_area_personale']) and $options['fbf_link_area_personale'] != "") {
						?>
							<a href="<?= $options['fbf_link_area_personale'] ?>" class="headline-icon">
								<img src="<?php echo get_stylesheet_directory_uri(); ?>/img/fbf-icons/svg/054-lock.svg" alt="Area Riservata Personale Fatebenefratelli" title="Area Riservata Personale Fatebenefratelli" />
							</a>
						<?php
						} ?>
							<a href="#s" class="headline-icon">
								<img src="<?php echo get_stylesheet_directory_uri(); ?>/img/fbf-icons/svg/057-loupe.svg" alt="Cerca nel sito" title="Cerca nel sito" id="ricercaNelSito"/>
							</a>
					</div>
					<div class="clear"></div>
				</div>
