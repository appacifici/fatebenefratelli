...
?>
	<div <?php post_class('post-full'); ?> id="post-<?php the_ID(); ?>">
		<div class="post-grs-title entry-title"><?php if (get_post_format() == 'quote') echo '&ldquo;' . get_post_meta($post->ID, OM_THEME_SHORT_PREFIX . 'quote', true);
												else the_title();	?></div>
		<div class="post-meta">
			...
			<?php } ?>
			<?php if (get_field('articolo-attivita-ospedaliera')) {
				$idUO = get_field('articolo-attivita-ospedaliera');
			?>
				<div class="post-uo">
					<?php
					echo '<span class="label">Attività Ospedaliera: </span>';
					foreach ($idUO as $attivitaOspedaliera) {
						$uo = get_post($attivitaOspedaliera);
						echo '<a href="' . $uo->guid . '">' . $uo->nome_attivita . '</a>';
					}
					?>
				</div>
			<?php } ?>
				...
				<div class="post-date block-1 zero-mar" title="<?php the_time(get_option('date_format')); ?>">
					<div class="block-inner">
						<div class="post-date-inner">
							<div><?php the_time('d'); ?></div><?php the_time('M'); ?>
						</div>
					</div>
				</div>
